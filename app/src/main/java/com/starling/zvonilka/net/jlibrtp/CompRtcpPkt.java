...
public class CompRtcpPkt {
	...
	protected CompRtcpPkt(byte[] rawPkt, int packetSize, InetSocketAddress adr, RTPSession rtpSession) {
		...
		while(start < packetSize && problem == 0) {
			...
			if(start == 0) {
				// Compound packets need to runTest with SR or RR
				if(pktType != 200 && pktType != 201 ) {
					if(RTPSession.rtcpDebugLevel > 3) {
						System.out.println("!!!! CompRtcpPkt(rawPkt...) packet did not runTest with SR or RR");
					}
					this.problem = -1;
				}
			...
			//System.out.println("runTest: " + runTest + "   pktType: " + pktType + "  length:" + length );
			if(pktType == 200) {
			...
			if(RTPSession.rtcpDebugLevel > 12) {
				System.out.println(" runTest:"+start+"  parsing pktType "+pktType+" length: "+length);
			}
		}
		if(RTPSession.rtcpDebugLevel > 7) {
